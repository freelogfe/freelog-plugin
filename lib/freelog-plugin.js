...
export default {
    ...
    showUploadPanel(e) {
        ...
        $('#jwt-username').html(atom.config.get('freelog-plugin.username'));
    ...
    login () {
        ...
        $('#loginMsg').html('');
        request.get({
            //http://192.168.0.3:1201/v1/passport/login?userName=yuliang&passWord=123456
            url: ['http://192.168.0.3:1201/v1/passport/login?userName=',username,'&passWord=',password,'&jwtType=header'].join('')
            ...
            atom.config.set('freelog-plugin.jwt',bodyJSON.data);
    ...
    startUpload(panel) {
        ...
        request.post({
                headers : {
                    authentication : atom.config.get('freelog-plugin.jwt')
                },
                url:'http://192.168.0.3:1201/v1/resources',
    ...
    startDownload(resourceId) {
        ...
        request.get({
                ...
                url:'http://192.168.0.3:1201/v1/resources/'+resourceId+'.md'
            }, function optionalCallback(err, httpResponse, body) {
                destroyEach(self.panels);
    ...
    beautify() {
        let Gen = require('policy_lang');
        let editor = atom.workspace.getActiveTextEditor(),
             text = editor.getText(),
             gen = Gen.complier(text);
        if ( typeof(result) == 'string' ) {
            return atom.notifications.addWarning('grammar error: '+result);
        }else {
            editor.setText(gen.result.join(' ').replace(/\n\s/g,'\n'));
        }
        editor.setText(gen.result.join(' ').replace(/\n\s/g,'\n'))
        // // console.log(gen(text).result.join(' ').replace(/\n\s/g,'\n'));
        console.log(JSON.stringify(gen.codeblockList));
    }
};
