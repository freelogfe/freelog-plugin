...
export default {
    ...
    jwtBinding() {
        ...
        //渲染policy title
        let policyJSON = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../policy-text/policy.json'), 'utf-8'));
...
//上传
    ...
    showPolicyList() {
      ...
      let liOpt = '';
      policyJSON.forEach(function (obj, index){
        liOpt += '<li  class="li-opts" value="'+ index +'">'+ obj.name +'</option>'
      })
    ...
    policyManagementBinding() {
      ...
      $('#policy-name-ul').off('click');
      $('#policy-name-ul').on('click', '.li-opts', function() {
        // policyJSON = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../policy-text/policy.json'), 'utf-8'));
        // $('.li-opts').removeClass('selected-policy');
        // $(this).addClass('selected-policy');
        // index = $(this).val()
        // var choosed = policyJSON[$(this).val()];
        // $('#policy-name-input').val(choosed.name);
        // $('#policy-text-textarea').val(choosed.text);
      })
      $('#save-policy').on('click', function() {
        // policyJSON = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../policy-text/policy.json'), 'utf-8'));
        // policyJSON[index].name = $('#policy-name-input').val();
        // policyJSON[index].text = $('#policy-text-textarea').val();
        // fs.writeFile(path.resolve(__dirname, '../policy-text/policy.json'), JSON.stringify(policyJSON), function (err) {
        //    if(err) {
        //       console.error(err);
        //     } else {
        //       $('.li-opts')[index].innerHTML = $('#policy-name-input').val();
        //     }
        // });
      });
      $('#new-policy').on('click', function() {
        // policyJSON = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../policy-text/policy.json'), 'utf-8'));
        // index = policyJSON.length;
        // policyJSON[policyJSON.length] = {
        //   name: 'new policy',
        //   text: 'write your policy'
        // };
        // $('#policy-name-input').val('new policy');
        // $('#policy-text-textarea').val('write your policy');
        // $('.li-opts').removeClass('selected-policy');
        // $('#policy-name-ul').append('<li  class="selected-policy li-opts" value="'+ index +'">new policy</option>')
        //
        // fs.writeFile(path.resolve(__dirname, '../policy-text/policy.json'), JSON.stringify(policyJSON), function (err) {
        //    if(err) {
        //       console.error(err);
